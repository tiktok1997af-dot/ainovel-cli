...
func TestRunBrowserVerificationAuthNormalLoginReadyRestartReady(t *testing.T) {
	...
	fake := &fakeVerificationSession{starts: []SessionSnapshot{
	...
	result, err := RunBrowserVerification(context.Background(), VerificationConfig{
		Site: "gemini-web", BrowserPath: browserPath, ProfileName: "w2-test",
		ProfileDir: profileDir, EvidencePath: evidencePath,
		PollInterval: time.Millisecond, LoginTimeout: 100 * time.Millisecond,
		RestartTimeout: 100 * time.Millisecond, RestartDelay: time.Millisecond,
		OnEvent: func(event VerificationEvent) { events = append(events, event) },
		sessionFactory: func(SessionConfig) verificationSession { return fake },
		loginLauncher: loginLauncher,
		now: func() time.Time { return time.Unix(100, 0).UTC() },
	})
	if err != nil { t.Fatalf("RunBrowserVerification: %v", err) }
	...
	if result.Evidence.Result != "PASS_RESTART_READY" { t.Fatalf("result = %q", result.Evidence.Result) }
	if result.Evidence.Schema != "ainovel.webai.w2e.v2" { t.Fatalf("schema = %q", result.Evidence.Schema) }
	if len(events) < 7 { t.Fatalf("events = %d, want at least 7", len(events)) }
	loginCfg := loginLauncher.lastConfig()
	if !loginCfg.DisableDevTools { t.Fatal("manual login browser must disable DevTools") }
	if loginCfg.ProfileDir != profileDir || loginCfg.StartURL != "https://gemini.google.com/app" { t.Fatalf("unexpected normal login config: %+v", loginCfg) }
	data, err := os.ReadFile(evidencePath)
	if err != nil { t.Fatal(err) }
	text := string(data)
	for _, forbidden := range []string{"cookie", "token", "localStorage", profileDir} {
		if strings.Contains(text, forbidden) { t.Fatalf("evidence leaked forbidden value %q: %s", forbidden, text) }
	}
}

func TestRunBrowserVerificationRejectsExistingProfile(t *testing.T) {
	profileDir := filepath.Join(t.TempDir(), "profile")
	if err := os.MkdirAll(profileDir, 0o700); err != nil { t.Fatal(err) }
	if err := os.WriteFile(filepath.Join(profileDir, "Cookies"), []byte("browser-owned"), 0o600); err != nil { t.Fatal(err) }
	_, err := RunBrowserVerification(context.Background(), VerificationConfig{ProfileDir: profileDir})
	if err == nil || !strings.Contains(err.Error(), "not clean") { t.Fatalf("err = %v, want clean-profile rejection", err) }
}

func TestRunBrowserVerificationFreshProfileCannotStartReady(t *testing.T) {
	fake := &fakeVerificationSession{starts: []SessionSnapshot{{State: SessionReady, Reason: "unexpected prior login"}}}
	_, err := RunBrowserVerification(context.Background(), VerificationConfig{
		ProfileDir: filepath.Join(t.TempDir(), "profile"), EvidencePath: filepath.Join(t.TempDir(), "evidence.json"),
		sessionFactory: func(SessionConfig) verificationSession { return fake },
	})
	if err == nil || !strings.Contains(err.Error(), "unexpectedly started READY") { t.Fatalf("err = %v", err) }
}

func TestVerificationEvidenceDoesNotContainSessionPathsOrSecrets(t *testing.T) {
	evidence := VerificationEvidence{Schema: verificationEvidenceSchema, Site: "gemini-web", ProfileName: "safe-name", Result: "PASS_RESTART_READY", Events: []VerificationEvent{{Sequence: 1, State: SessionReady, Reason: "authenticated Gemini composer is ready"}}}
	path := filepath.Join(t.TempDir(), "evidence.json")
	if err := writeVerificationEvidence(path, evidence); err != nil { t.Fatal(err) }
	data, err := os.ReadFile(path)
	if err != nil { t.Fatal(err) }
	for _, forbidden := range []string{"browser_path", "profile_dir", "cookie", "auth_token", "local_storage"} {
		if strings.Contains(strings.ToLower(string(data)), forbidden) { t.Fatalf("evidence contains forbidden field %q: %s", forbidden, data) }
	}
}
