...
func newDesktopSubscription(capacity int, onClose func()) *desktopSubscription {
	...
	return &desktopSubscription{events: make(chan DesktopEvent, capacity), onClose: onClose}
}
...
// offer never waits for a desktop consumer. If a subscriber falls behind, the
// oldest buffered item is dropped in favor of the newest state/prose delta.
// Durable events remain recoverable by reconnecting with the last seen Seq.
...
func (r *Runtime) startEventHub() {
	r.eventHubOnce.Do(func() {
		...
		r.eventMu.Unlock()
		go func() { defer close(done); r.runEventHub(ctx) }()
	})
}
...
func (r *Runtime) stopEventHub() {
	...
	for _, sub := range r.subscribers { subs = append(subs, sub) }
	r.subscribers = make(map[uint64]*desktopSubscription)
	r.eventMu.Unlock()
	if cancel != nil { cancel() }
	if done != nil { <-done }
	for _, sub := range subs { _ = sub.Close() }
}
	...
	for {
		...
		case delta, ok := <-streamCh:
			...
			case utils.ThinkingSep:
				if streamMode == "thinking" { streamMode = "content" } else { streamMode = "thinking" }
		...
		case <-ticker.C:
			...
			if err != nil { slog.Warn("desktop runtime queue tail failed", "module", "appruntime.events", "after_seq", lastDurableSeq, "err", err); continue }
			for _, item := range items {
				if item.Seq <= lastDurableSeq { continue }
				r.broadcast(projectRuntimeQueueItem(item)); lastDurableSeq = item.Seq
			}
		}
	}
}
...
func (r *Runtime) broadcast(ev DesktopEvent) {
	r.eventMu.Lock(); defer r.eventMu.Unlock()
	for _, sub := range r.subscribers { sub.offer(ev) }
}

func (r *Runtime) registerSubscription(sub *desktopSubscription) uint64 {
	id := r.nextSubscriberID.Add(1)
	r.eventMu.Lock(); if r.subscribers == nil { r.subscribers = make(map[uint64]*desktopSubscription) }; r.subscribers[id] = sub; r.eventMu.Unlock()
	return id
}

func (r *Runtime) removeSubscription(id uint64) { r.eventMu.Lock(); delete(r.subscribers, id); r.eventMu.Unlock() }

func projectHostEvent(ev host.Event, typ string, seq int64) DesktopEvent {
	payload, _ := json.Marshal(ev)
	return DesktopEvent{ContractVersion: ContractVersion, Seq: seq, Time: utcTime(ev.Time), Category: ev.Category, Type: typ, Level: ev.Level, Summary: ev.Summary, Payload: payload}
}
...
func projectRuntimeQueueItem(item domain.RuntimeQueueItem) DesktopEvent {
	if ev, ok := decodeRuntimeHostEvent(item.Payload); ok {
		...
		if out.Time.IsZero() { out.Time = utcTime(item.Time) }
		if out.Category == "" { out.Category = item.Category }
		if out.Summary == "" { out.Summary = item.Summary }
		out.TaskID = item.TaskID
		return out
	}
	payload, _ := json.Marshal(item.Payload)
	return DesktopEvent{ContractVersion: ContractVersion, Seq: item.Seq, Time: utcTime(item.Time), Category: item.Category, Type: eventTypeDurable, TaskID: item.TaskID, Summary: item.Summary, Payload: payload}
}

func decodeRuntimeHostEvent(payload any) (host.Event, bool) {
	if payload == nil { return host.Event{}, false }
	if ev, ok := payload.(host.Event); ok { return ev, true }
	data, err := json.Marshal(payload); if err != nil { return host.Event{}, false }
	var ev host.Event
	if err := json.Unmarshal(data, &ev); err != nil { return host.Event{}, false }
	if ev.Category == "" && ev.Summary == "" && ev.ID == "" { return host.Event{}, false }
	return ev, true
}
