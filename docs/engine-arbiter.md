...
> Engine + Arbiter design record is retained as provenance only. Current product authority is `README.md` + `docs/architecture.md`. Provider failover, API prompt-cache/usage-budget semantics and runtime model switching described below are not current product behavior.
...
> 本文档保留为设计决策记录;当前架构见 README 架构节与 docs/architecture.md。关联:docs/voice-layer.md(已实施)。
